...
4. Exclude transfers from spending totals unless the user explicitly asks about them: add `WHERE is_internal = false AND category != 'Transfer'`. `is_internal` alone often isn't enough — it's only set when a matching transaction was found on *another* account (e.g. the checking-account side of a credit card payment), which requires the user to have uploaded both accounts. A single-statement upload (the common case) will have real transfers/card payments correctly categorized `'Transfer'` but never matched, so `is_internal` stays false for them — filtering on category too catches those.
